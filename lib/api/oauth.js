...
// Currently only 'Server' (and 'Existing') is working.
/**
...
module.exports = {
    ...
    * @private
    */
    requiredOptions: ['user'],
    Password: function(options) {
        requiredOptions.concat(['pass', 'clientId']);
        if (err = util.validateOptions(options, this.requiredOptions)) throw err;
    ...
    */
    Server: function(options) {
        requiredOptions.concat(['clientId', 'clientSecret', 'redirectUri']);
        if (err = util.validateOptions(options, this.requiredOptions)) throw err;
    ...
    Application: function(options) {
        requiredOptions.concat(['clientId', 'clientSecret']);
        if (err = util.validateOptions(options, this.requiredOptions)) throw err;
    ...
    * @param {String} options.user The user's email used to login to Conversations.
    * redirect the user after authorization.
    * @return {Object} A completed OAuth object if found, otherwise a partially initialized OAuth object.
    * @throws {OptionError} If one or more of the required options is not supplied.
    * @example
    *     var options = {
    *         user: <user>,
