var app;
var spredfast = require('../spredfast.js');
...
var required = ['company','initiative','accounts','service','content'];
...
* @TODO Allow additional 22 characters for image tweets
...
var customValidation = function(options) {
    ...
    if (!util.validEnum(services, options.service)) {
        var validServices = 'spredfast.services.[' + Object.keys(services).join() + ']';
        return new ContentError('options.service must be one of ' + validServices);
    }

    var invalidAccounts = options.accounts.filter(function(account) {
    ...
    if (!util.validEnum(contentTypes, options.content.sfEntityType)) {
        var validContentTypes = 'spredfast.contentTypes.[' + Object.keys(contentTypes).join() + ']';
        return new ContentError('options.content.sfEntityType must be one of ' + validContentTypes);
    }

    var isStatus = options.content.sfEntityType === contentTypes.STATUS;
    var requiredContent = ['sfEntityType', isStatus ? 'text' : 'imagePath'];
    ...
    } else if (util.fileSizeInMegaBytes(options.content.imagePath) >= 1000) {
...
var setupCallback = function(callback) {
    ...
    var uri = 'http://callbacktest1.jloehr.ultrahook.com?id=' + app.routeFns.length;
